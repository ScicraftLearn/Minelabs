...
public class AutomatedLaserBlock extends BlockWithEntity implements Waterloggable {
    private static final DirectionProperty FACING = Properties.HORIZONTAL_FACING;
    private static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public AutomatedLaserBlock(Settings settings) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new AutomatedLaserBlockEntity(pos, state);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        ...
        return checkType(type, BlockEntities.AUTOMATED_LASER_BLOCK_ENTITY, (world1, pos, state1, blockEntity) -> {
            if (!world1.isClient()) {
